const Users = () => {	
	...
	const signup_validate = function (username, email, phone, pwd) {
		if (!username || !email || !phone || !pwd) return 0;
	...
	const signup = function () {
		const log = [
			...
			'That username, email or phone already exists'
			];
		...
		const validation = signup_validate(username, email, phone, pwd);
		if (validation != 1) {
			switch (validation) {
				case 0: return; break;
			}
			...
			fetch('assets/php/client_signup_createAccount.php', {
			  method: 'POST',
			  body: formData
			})
	...
	const check_input = Debounce(function (e, field) {
		...
		else if (field == 2 && value.length != 10) state = -1
		else state = 1;
